...
The PMF defines the probability of an outcome - for a a large number of discreet outcomes, we could end up with a large number of possible outcomes.  So we use some shortcuts - mean and spread or variance, which is the mean of squared differences from the mean).  We can also use a chart e.g. a histogram to see the distribution.  
